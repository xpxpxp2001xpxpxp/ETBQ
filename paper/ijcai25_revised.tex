...
\begin{itemize}
    ...
    \item We provide a convergence analysis showing that ETBQ optimizes a smoothed surrogate objective under standard non-convex stochastic optimization assumptions.
...
Given the modeled distribution $\mathcal{N}(\boldsymbol{\mu}_w, \boldsymbol{\Sigma}_w)$, directly adding i.i.d. noise at every step would introduce a persistent mean shift when $\boldsymbol{\mu}_w \neq \mathbf{0}$. ETBQ instead uses a temporally anti-correlated differential mechanism. At step $t$, we construct the temporary evaluation weight
\begin{equation} \label{eqt:diff_noise_def}
   \boldsymbol{W}'_t = \widetilde{\boldsymbol{W}}_t + \lambda_e \cdot \boldsymbol{\delta}_{t} - \boldsymbol{\delta}_{t-1},
\end{equation}
where $\widetilde{\boldsymbol{W}}_t$ is the physical trainable weight, $\boldsymbol{\delta}_{t}$ and $\boldsymbol{\delta}_{t-1}$ are consecutive samples from $\mathcal{N}(\boldsymbol{\mu}_w, \boldsymbol{\Sigma}_w)$, and $\lambda_e \in [0,1]$ is an epoch-dependent annealing coefficient. As $\lambda_e$ approaches 1, the expected differential perturbation approaches zero. Over the full trajectory, the accumulated bias forms a telescoping sum and becomes negligible in the time average.

To expose the effective objective, define a latent clean weight $\boldsymbol{W}_t \triangleq \widetilde{\boldsymbol{W}}_t - \lambda_e \boldsymbol{\delta}_t$. The physical update with Eq.~\eqref{eqt:diff_noise_def} is equivalent to evaluating gradients at a perturbed latent point:
\begin{equation}
\label{eqt:latent_trajectory}
\boldsymbol{W}_{t+1} = \boldsymbol{W}_{t} - \eta \nabla \mathcal{L}(\boldsymbol{W}_{t} + \lambda_e \boldsymbol{\delta}_{t+1}),
\end{equation}
where $\eta$ is the learning rate. Thus WQN optimizes a stochastically smoothed objective while avoiding persistent drift in the physical weights. This derivation assumes SGD-style isotropic updates; extending the same mechanism to strongly anisotropic adaptive optimizers such as Adam is left for future work.
...
\[
\tilde{\mathcal{L}}(\boldsymbol{W}) = \mathbb{E}_{\boldsymbol{\delta}}[\mathcal{L}(\boldsymbol{W} + \lambda_e \boldsymbol{\delta})].
\]
Decompose $\boldsymbol{\delta} = \boldsymbol{\mu}_w + \boldsymbol{\epsilon}$, where $\boldsymbol{\epsilon} \sim \mathcal{N}(\mathbf{0}, \boldsymbol{\Sigma}_w)$. A second-order Taylor expansion around $\boldsymbol{W} + \lambda_e \boldsymbol{\mu}_w$ gives
\begin{equation}
\label{eqt:effective_loss_final}
\tilde{\mathcal{L}}(\boldsymbol{W}) \approx \underbrace{\mathcal{L}(\boldsymbol{W} + \lambda_e \boldsymbol{\mu}_w)}_{\text{Targeting the quantized center}} + \underbrace{\frac{\lambda_e^2}{2} \mathrm{Tr}(\boldsymbol{H}_{w,\mu} \boldsymbol{\Sigma}_w)}_{\text{Flattening the specific landscape}},
\end{equation}
where $\boldsymbol{H}_{w,\mu} = \nabla^2 \mathcal{L}(\boldsymbol{W} + \lambda_e \boldsymbol{\mu}_w)$. This objective has two desirable effects: it shifts optimization toward the expected deployed state and penalizes curvature along directions where quantization error variance is large. WQN therefore does not flatten the landscape uniformly; it aligns flatness with the target quantization grid.
...
\section{Proof of Convergence for ETBQ}
\label{app:proof}

We provide a convergence analysis for ETBQ under the standard framework of non-convex stochastic optimization. Let $\mathcal{L}(\boldsymbol{W})$ denote the task loss, and let ETBQ optimize the Gaussian-smoothed surrogate
\[
\mathcal{L}_\sigma(\boldsymbol{W})=\mathbb{E}_{\boldsymbol{\delta}}[\mathcal{L}(\boldsymbol{W}+\boldsymbol{\delta})],
\]
where $\boldsymbol{\delta}$ collects the effective WQN and AQN perturbations. The proof abstracts away architectural details and focuses on the stochastic gradients induced by the perturbation process.
...
\end{equation}
This assumption is natural because randomized smoothing by continuous perturbations mollifies the non-smooth effects introduced by quantization-like operators.

\textbf{Assumption 2 (Lower Bounded Objective).}
There exists $\mathcal{L}_\sigma^* > -\infty$ such that $\mathcal{L}_\sigma(\boldsymbol{W}) \geq \mathcal{L}_\sigma^*$ for all $\boldsymbol{W}$. For cross-entropy training, the loss is non-negative, so this condition holds.

\textbf{Assumption 3 (Asymptotically Unbiased Gradient and Bounded Variance).}
Let $\tilde{\boldsymbol{g}}_t$ be the stochastic gradient used by ETBQ and $\boldsymbol{g}_t=\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)$. The differential WQN term has time-averaged drift that vanishes by telescoping:
\begin{equation}
    \frac{1}{T}\sum_{t=1}^{T}\mathbb{E}[\boldsymbol{\delta}_{w,t}-\boldsymbol{\delta}_{w,t-1}]
    =\frac{\mathbb{E}[\boldsymbol{\delta}_{w,T}]-\mathbb{E}[\boldsymbol{\delta}_{w,0}]}{T}
    \rightarrow \mathbf{0}.
\end{equation}
We therefore treat the trajectory-averaged gradient estimator as unbiased for the smoothed objective and assume bounded variance:
\begin{equation}
    \mathbb{E}\left[\|\tilde{\boldsymbol{g}}_t-\boldsymbol{g}_t\|^2\right]\leq \sigma_{eff}^2.
\end{equation}

\begin{theorem}
\textit{Under Assumptions 1--3, the ETBQ optimization sequence $\{\boldsymbol{W}_t\}_{t=1}^T$ generated by SGD with learning rate $\eta \leq \frac{1}{L_\sigma}$ satisfies}
\begin{equation}
...
\end{equation}
\textit{Consequently, with a standard diminishing learning rate, ETBQ converges to a first-order stationary point of the smoothed objective in expectation.}
\end{theorem}

\begin{proof}
By $L_\sigma$-smoothness and the SGD update $\boldsymbol{W}_{t+1}=\boldsymbol{W}_t-\eta\tilde{\boldsymbol{g}}_t$, the descent lemma gives
\begin{equation}
...
\end{equation}
Taking expectation and using the trajectory-unbiased property,
\begin{equation}
    \mathbb{E}\left[\langle \nabla \mathcal{L}_\sigma(\boldsymbol{W}_t),\tilde{\boldsymbol{g}}_t\rangle\right]
    =
    \mathbb{E}\left[\|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2\right].
\end{equation}
The bounded-variance condition implies
\begin{equation}
    \mathbb{E}\left[\|\tilde{\boldsymbol{g}}_t\|^2\right]
    \leq
    \mathbb{E}\left[\|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2\right]+\sigma_{eff}^2.
\end{equation}
Substituting these two relations yields
\begin{equation}
\begin{aligned}
    \mathbb{E}[\mathcal{L}_\sigma(\boldsymbol{W}_{t+1})]
    &\leq
    \mathbb{E}[\mathcal{L}_\sigma(\boldsymbol{W}_t)]
    -\eta\left(1-\frac{L_\sigma\eta}{2}\right)
    \mathbb{E}\left[\|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2\right]
    +\frac{L_\sigma\eta^2}{2}\sigma_{eff}^2.
\end{aligned}
\end{equation}
...
\begin{aligned}
    ...
    -\mathbb{E}[\mathcal{L}_\sigma(\boldsymbol{W}_{T+1})]
    +\frac{L_\sigma\eta^2T}{2}\sigma_{eff}^2 \\
    &\leq
    \mathcal{L}_\sigma(\boldsymbol{W}_1)-\mathcal{L}_\sigma^*
    +\frac{L_\sigma\eta^2T}{2}\sigma_{eff}^2.
\end{aligned}
\end{equation}
