...
\section{Convergence Analysis of the WQN Update}
\label{app:proof}
...
\begin{equation}
    \mathcal{L}_\sigma(\boldsymbol{W})=\mathbb{E}_{\boldsymbol{\delta}}[\mathcal{L}(\boldsymbol{W}+\boldsymbol{\delta})],
\end{equation}
where $\boldsymbol{\delta}$ collects the effective weight quantization perturbations. The proof abstracts away specific architectural details and focuses on the stochastic gradients induced by the perturbation process.
...
There exists $\mathcal{L}_\sigma^* > -\infty$ such that $\mathcal{L}_\sigma(\boldsymbol{W}) \geq \mathcal{L}_\sigma^*$ for all $\boldsymbol{W}$. For cross-entropy training, the loss is non-negative, so this condition inherently holds.
