...
\item We provide a convergence analysis for the WQN update, showing that temporal differential weight-noise injection can be analyzed as optimization over a smoothed surrogate objective under standard non-convex stochastic optimization assumptions.
...
Given the modeled distribution $\mathcal{N}(\boldsymbol{\mu}_w, \boldsymbol{\Sigma}_w)$, directly adding i.i.d. noise at every step would introduce a persistent mean shift when $\boldsymbol{\mu}_w \neq \mathbf{0}$. ETBQ instead uses a temporally differential mechanism. At step $t$, we first draw an unscaled quantization-noise sample $\boldsymbol{\epsilon}_{t}$ from the current estimated distribution and absorb the annealing coefficient into the actual injected noise,
\[
\boldsymbol{\epsilon}_{t}\sim \mathcal{N}(\boldsymbol{\mu}_{w,t},\boldsymbol{\Sigma}_{w,t}),
\qquad
\boldsymbol{\delta}_{t}=\lambda_e\boldsymbol{\epsilon}_{t}.
\]
The temporary evaluation weight is then constructed by replacing the previous injected noise with the current one:
\begin{equation} \label{eqt:diff_noise_def}
   \boldsymbol{W}'_t = \widetilde{\boldsymbol{W}}_t + \boldsymbol{P}_t,
   \qquad
   \boldsymbol{P}_t = \boldsymbol{\delta}_{t}-\boldsymbol{\delta}_{t-1},
\end{equation}
where $\widetilde{\boldsymbol{W}}_t$ denotes the physical trainable weight before the current forward pass. This formulation matches the implementation: the model stores the previous perturbation and adds only the difference between the new and old perturbations. Since the statistics of $\boldsymbol{\delta}_{t}$ may change slowly with the weights, $\boldsymbol{P}_t$ is not assumed to be an i.i.d. zero-mean sample. Instead, the differential form prevents progressive drift in the time average:
\[
\frac{1}{T}\sum_{t=1}^{T}\mathbb{E}[\boldsymbol{P}_t]
=
\frac{\mathbb{E}[\boldsymbol{\delta}_{T}]-\mathbb{E}[\boldsymbol{\delta}_{0}]}{T}
\rightarrow \mathbf{0},
\]
provided that the quantization-noise mean remains bounded. This trajectory-level cancellation is the key reason why WQN is more stable than naive additive noise.

Following this drift-controlled view, the stochastic gradient evaluated at $\boldsymbol{W}'_t$ is treated as an idealized conditionally unbiased estimator of the gradient of a WQN-smoothed objective:
\begin{equation}
\label{eqt:latent_trajectory}
\mathbb{E}_{t}\left[\nabla \mathcal{L}(\widetilde{\boldsymbol{W}}_t+\boldsymbol{P}_t)\right]
=
\nabla \mathcal{L}_{\sigma}(\widetilde{\boldsymbol{W}}_t),
\end{equation}
where $\mathcal{L}_{\sigma}(\boldsymbol{W})=\mathbb{E}_{\boldsymbol{\delta}}[\mathcal{L}(\boldsymbol{W}+\boldsymbol{\delta})]$ denotes the Gaussian-smoothed weight-space objective. This condition is explicitly stated in Appendix~\ref{app:proof}; it is an analysis assumption motivated by the telescoping drift cancellation rather than a claim that every individual step is exactly unbiased. The derivation assumes SGD-style updates; extending the same mechanism to strongly anisotropic adaptive optimizers such as Adam is left for future work.

\subsubsection{Regularizing Hessian Trace}
\label{sec:wqn_hessian}

Equation~\eqref{eqt:latent_trajectory} implies that WQN approximately minimizes a Gaussian-smoothed weight-space objective
\[
\mathcal{L}_{\sigma}(\boldsymbol{W}) = \mathbb{E}_{\boldsymbol{\delta}}[\mathcal{L}(\boldsymbol{W} + \boldsymbol{\delta})],
\qquad
\boldsymbol{\delta}\sim\mathcal{N}(\boldsymbol{\mu}_w,\boldsymbol{\Sigma}_w).
\]
To expose the curvature regularization effect, decompose $\boldsymbol{\delta}=\boldsymbol{\mu}_w+\boldsymbol{\epsilon}$ with $\boldsymbol{\epsilon}\sim\mathcal{N}(\mathbf{0},\boldsymbol{\Sigma}_w)$. A second-order Taylor expansion around the expected perturbed state $\boldsymbol{W}+\boldsymbol{\mu}_w$ gives
\begin{equation}
\label{eqt:effective_loss_final}
\mathcal{L}_{\sigma}(\boldsymbol{W}) \approx \underbrace{\mathcal{L}(\boldsymbol{W}+\boldsymbol{\mu}_w)}_{\text{Expected perturbed state}} + \underbrace{\frac{1}{2}\mathrm{Tr}(\boldsymbol{H}_{w,\mu}\boldsymbol{\Sigma}_w)}_{\text{Quantization-aligned curvature penalty}},
\end{equation}
where $\boldsymbol{H}_{w,\mu}=\nabla^2\mathcal{L}(\boldsymbol{W}+\boldsymbol{\mu}_w)$. If the annealing coefficient is written explicitly, $\boldsymbol{\mu}_w$ and $\boldsymbol{\Sigma}_w$ are replaced by $\lambda_e\boldsymbol{\mu}_w$ and $\lambda_e^2\boldsymbol{\Sigma}_w$, respectively. Therefore, WQN does not flatten the landscape uniformly; it penalizes curvature most strongly along directions where the target quantizer produces larger weight errors, while the differential update prevents these perturbations from accumulating as a persistent drift.
...
ETBQ is a task-agnostic pre-conditioning framework. Its goal is not to replace the original task objective, but to optimize the full-precision model under statistically modeled quantization perturbations before PTQ is applied. For a minibatch $(\boldsymbol{x},y)\in\mathscr{D}_{t}$, let $\mathcal{L}_{task}(\cdot,\cdot)$ denote the task-specific training loss. For image classification, $\mathcal{L}_{task}$ is typically cross-entropy; for dense prediction tasks such as semantic segmentation, it can be the corresponding pixel-level supervision loss. ETBQ keeps this original supervision unchanged.

During ETBQ tuning, the model is evaluated with temporarily perturbed weights and intermediate activations. Specifically, WQN constructs the perturbed weights $\boldsymbol{W}'$ by temporal differencing in Eq.~\eqref{eqt:diff_noise_def}, while AQN injects masked activation perturbations according to Eq.~\eqref{eqt:aqn_stoch_injection}. The general ETBQ objective is therefore written as
\begin{equation}
\label{eqt:main_loss}
    \mathcal{L}
    =
    \mathcal{L}_{task}
    \left(
    f(\boldsymbol{x};\boldsymbol{W}',\mathrm{AQN}),
    y
    \right),
\end{equation}
where $f(\boldsymbol{x};\boldsymbol{W}',\mathrm{AQN})$ denotes the forward pass using WQN-perturbed weights and AQN-perturbed intermediate activations.

The perturbation strength $\lambda_e$ is gradually increased during training. This warm-up schedule avoids disrupting the full-precision model at the beginning of tuning and allows the network to progressively adapt to the target low-bit perturbation distribution. In the final stage, ETBQ applies Stochastic Weight Averaging (SWA) to aggregate the late-stage trajectory and obtain a solution closer to the center of a flat basin. Since SWA changes the effective network parameters, BatchNorm statistics are recomputed before exporting the model for PTQ.

Algorithm~\ref{alg:etbq} summarizes the complete procedure. The output of ETBQ is still a full-precision model, but it has been pre-conditioned to be more robust to subsequent post-training quantization. Therefore, it can be directly combined with existing PTQ backends such as QDrop~\cite{wei-qdrop-arxiv-2022}.
...
All experiments are implemented in PyTorch~\cite{paszke-pytorch-nips-2019}, with MQBench~\cite{li-mqbench-arxiv-2021} used as the PTQ backend. Unless otherwise specified, we apply asymmetric uniform quantization following QDrop~\cite{wei-qdrop-arxiv-2022}. We keep the first and last layers at 8-bit precision to avoid excessive information loss at the input and output boundaries. We use per-channel quantization for weights and per-tensor quantization for activations, consistent with the quantization-error modeling in Section~\ref{sec:method}. We use W$X$A$Y$ to denote $X$-bit weight and $Y$-bit activation quantization.

\textbf{PTQ Calibration.}
For each dataset, the calibration set is randomly sampled from the corresponding training split and kept fixed for all compared PTQ methods. Unless otherwise stated, CIFAR-100~\cite{krizhevsky-cifar-arxiv-2009} uses 100 calibration images, Tiny-ImageNet uses 200 calibration images, and Cityscapes uses 128 calibration images. The same calibration samples are used for quantization to ensure a fair comparison.

\textbf{ETBQ Pre-conditioning.}
ETBQ is applied before PTQ as a lightweight full-precision pre-conditioning stage. During this stage, WQN and AQN statistics are estimated from the calibration set and updated during tuning as described in Section~\ref{sec:method}. After ETBQ tuning, the resulting full-precision model is passed to the same PTQ backend as the baselines. Therefore, the performance gain comes from improving the quantization-friendliness of the full-precision model rather than changing the PTQ reconstruction algorithm.

\textbf{Evaluation Metrics.}
For image classification on CIFAR-100 and Tiny-ImageNet, we report top-1 accuracy. For semantic segmentation on Cityscapes, we report mean Intersection-over-Union (mIoU). For each quantized model, we also report the performance drop relative to its corresponding FP32 model when appropriate.
...
\textbf{Setting.}
We conduct ablation studies on CIFAR-100 with ResNet-18 under the extreme W2A4 quantization setting. The baseline is a standard SGD-trained full-precision model followed by QDrop PTQ. ETBQ uses SGD with momentum 0.9, batch size 256, weight decay 0.0005, and an initial learning rate of 0.015 with cosine annealing. We train for 400 epochs. Noise injection starts at epoch 200 and linearly warms up for 50 epochs. SWA~\cite{DBLP:journals/corr/abs-1803-05407} is activated during the final 100 epochs. We use cross-entropy loss with label smoothing factor 0.1.

\subsubsection{Ablation Study of the ETBQ Framework}

\begin{table}[h]
\centering
\caption{Ablation study of ETBQ components on CIFAR-100 with W2A4 ResNet-18.}
...
Table~\ref{tab:ablation_components} reports the contribution of WQN, AQN, and SWA. The baseline drops from 79.40\% to 76.47\% after W2A4 quantization. Individually, WQN, AQN, and SWA improve W2A4 accuracy to 77.67\%, 77.37\%, and 77.22\%, respectively, showing that weight-side smoothing, activation-side robustness, and trajectory averaging are all beneficial. WQN gives the largest single-component improvement, indicating that weight-side landscape pre-conditioning is especially important under low-bit PTQ.

Combining WQN and AQN without SWA gives 77.43\%, which is lower than WQN alone. This suggests that jointly injecting heterogeneous perturbations can create a turbulent tuning trajectory. With SWA, the full ETBQ reaches 78.06\%, yielding a +1.59\% gain over the baseline. The result demonstrates that SWA effectively consolidates the noisy trajectory and is crucial for fully exploiting the benefits of WQN and AQN.
...
\subsection{Comparison with State-of-the-Art PTQ Methods}
\label{sec4.2}
...
\textbf{Setting.}
We evaluate ResNet-18, ResNet-50~\cite{he-resnet-cvpr-2016}, MobileNet-V1~\cite{howard-mbv1-arxiv-2017}, and MobileNet-V2~\cite{sandler-mbv2-cvpr-2018} on CIFAR-100. All PTQ methods use the same 100-image calibration set and the same quantization configuration. ETBQ uses QDrop as the PTQ backend after pre-conditioning. We compare against representative PTQ baselines including AdaRound~\cite{nagel-adaround-icml-2020}, BRECQ~\cite{li-brecq-arxiv-2021}, and QDrop~\cite{wei-qdrop-arxiv-2022}.
...
\textbf{Setting.}
To evaluate generalization beyond CIFAR-100, we conduct experiments on Tiny-ImageNet with ResNet-18. We sample 200 training images for PTQ calibration and report top-1 validation accuracy under FP32, W4A4, W2A4, and W2A2 settings.

As shown in Table~\ref{tbl:tiny-imagenet}, ETBQ consistently improves ResNet-18 on Tiny-ImageNet. The gain is modest in FP32 and W4A4, but becomes larger in more aggressive settings. In W2A4 and W2A2, ETBQ improves top-1 accuracy by +2.14\%. This trend is consistent with our hypothesis that landscape pre-conditioning becomes increasingly important when quantization perturbations are large enough for the geometry of the full-precision basin to become a limiting factor.

\subsubsection{Application to Semantic Segmentation.}
\textbf{Setting.}
We further evaluate ETBQ on semantic segmentation using U-Net~\cite{ronneberger2015u} on Cityscapes. We sample 128 training images for calibration and report mIoU on the validation set. Semantic segmentation is highly sensitive to spatial feature corruption, making it a challenging testbed for activation quantization robustness.
...
\section{Convergence Analysis of the WQN Injection}
\label{app:proof}

We provide a convergence analysis for the weight-space component of ETBQ, namely the Weight Quantization Noise (WQN) update, under the standard framework of non-convex stochastic optimization. Let $\mathcal{L}(\boldsymbol{W})$ denote the task loss, and let us consider the Gaussian-smoothed surrogate objective
\begin{equation}
    \mathcal{L}_\sigma(\boldsymbol{W})=\mathbb{E}_{\boldsymbol{P}}[\mathcal{L}(\boldsymbol{W}+\boldsymbol{P})],
\end{equation}
where $\boldsymbol{P}$ denotes the effective WQN perturbation. The proof abstracts away specific architectural details and focuses on the stochastic gradients induced by the perturbation process.

\textbf{Assumption 1 (Gradient Lipschitz Continuity).}
The smoothed objective $\mathcal{L}_\sigma(\boldsymbol{W})$ has $L_\sigma$-Lipschitz continuous gradients: for any $\boldsymbol{W}_1,\boldsymbol{W}_2$,
\begin{equation}
    \|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_1) - \nabla \mathcal{L}_\sigma(\boldsymbol{W}_2)\| \leq L_\sigma \|\boldsymbol{W}_1 - \boldsymbol{W}_2\|.
\end{equation}
This is a standard assumption in non-convex stochastic optimization. Gaussian smoothing motivates this assumption by reducing local irregularities and mitigating the non-smooth effects induced by quantization-like operators.

\textbf{Assumption 2 (Boundedness from Below).}
There exists $\mathcal{L}_\sigma^* > -\infty$ such that $\mathcal{L}_\sigma(\boldsymbol{W}) \geq \mathcal{L}_\sigma^*$ for all $\boldsymbol{W}$. This is the standard lower-boundedness condition for task losses used in non-convex stochastic optimization.

\textbf{Assumption 3 (Trajectory-Averaged Unbiasedness and Bounded Variance).}
Let $\tilde{\boldsymbol{g}}_t$ be the stochastic gradient used by the algorithm at step $t$, and let $\boldsymbol{g}_t=\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)$. In practice, the statistical distribution of the quantization noise shifts as weights update, introducing a slight local bias. However, the differential mechanism utilizes the difference between consecutive steps to ensure that the time-averaged drift vanishes macroscopically by telescoping:
\begin{equation}
    \frac{1}{T}\sum_{t=1}^{T}\mathbb{E}[\boldsymbol{\delta}_{t}-\boldsymbol{\delta}_{t-1}]
    =\frac{\mathbb{E}[\boldsymbol{\delta}_{T}]-\mathbb{E}[\boldsymbol{\delta}_{0}]}{T}
    \rightarrow \mathbf{0}.
\end{equation}
Since the weights evolve smoothly under a small learning rate, this local bias is expected to remain controlled. Motivated by this vanishing trajectory-averaged drift, and following standard stochastic optimization analyses, we adopt the following idealized conditionally unbiased gradient assumption for tractable analysis, alongside bounded variance:
\begin{equation}
    \mathbb{E}_t[\tilde{\boldsymbol{g}}_t] = \boldsymbol{g}_t, \quad \mathbb{E}_t\left[\|\tilde{\boldsymbol{g}}_t-\boldsymbol{g}_t\|^2\right]\leq \sigma_{eff}^2.
\end{equation}

\begin{theorem}
\textit{Under Assumptions 1--3, the optimization sequence $\{\boldsymbol{W}_t\}_{t=1}^T$ generated by SGD with learning rate $\eta \leq \frac{1}{L_\sigma}$ satisfies}
\begin{equation}
    \frac{1}{T} \sum_{t=1}^{T} \mathbb{E}\left[\|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2\right] \leq \frac{2(\mathcal{L}_\sigma(\boldsymbol{W}_1) - \mathcal{L}_\sigma^*)}{\eta T} + \eta L_\sigma \sigma_{eff}^2.
\end{equation}
\textit{Furthermore, by choosing a horizon-dependent constant stepsize $\eta = \min\{\frac{1}{L_\sigma}, \frac{c}{\sqrt{T}}\}$, the bound becomes $\mathcal{O}(\frac{1}{\sqrt{T}})$, which implies convergence to a first-order stationary point in expectation.}
\end{theorem}

\begin{proof}
Let $\mathcal{F}_t$ be the filtration generated by the sequence of random variables up to iteration $t$. Given $\mathcal{F}_t$, the weight $\boldsymbol{W}_t$ and the true gradient $\boldsymbol{g}_t = \nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)$ are deterministic. We denote the conditional expectation given $\mathcal{F}_t$ as $\mathbb{E}_t[\cdot]$.

By the $L_\sigma$-Lipschitz continuity of the gradient (Assumption 1), the descent lemma gives
\begin{equation}
    \mathcal{L}_\sigma(\boldsymbol{W}_{t+1}) \leq \mathcal{L}_\sigma(\boldsymbol{W}_t) + \langle \nabla \mathcal{L}_\sigma(\boldsymbol{W}_t), \boldsymbol{W}_{t+1}-\boldsymbol{W}_t \rangle + \frac{L_\sigma}{2} \|\boldsymbol{W}_{t+1}-\boldsymbol{W}_t\|^2.
\end{equation}
Substituting the SGD update rule $\boldsymbol{W}_{t+1} - \boldsymbol{W}_t = -\eta\tilde{\boldsymbol{g}}_t$, we obtain
\begin{equation}
    \mathcal{L}_\sigma(\boldsymbol{W}_{t+1}) \leq \mathcal{L}_\sigma(\boldsymbol{W}_t) - \eta\langle \nabla \mathcal{L}_\sigma(\boldsymbol{W}_t), \tilde{\boldsymbol{g}}_t\rangle + \frac{L_\sigma\eta^2}{2}\|\tilde{\boldsymbol{g}}_t\|^2.
\end{equation}
Taking the conditional expectation $\mathbb{E}_t[\cdot]$ on both sides and using Assumption 3 gives
\begin{equation}
    \mathbb{E}_t[\langle \nabla \mathcal{L}_\sigma(\boldsymbol{W}_t), \tilde{\boldsymbol{g}}_t\rangle] = \|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2.
\end{equation}
For the second moment term, the variance identity yields
\begin{equation}
\begin{aligned}
    \mathbb{E}_t[\|\tilde{\boldsymbol{g}}_t\|^2]
    &= \mathbb{E}_t[\|\tilde{\boldsymbol{g}}_t - \boldsymbol{g}_t + \boldsymbol{g}_t\|^2] \\
    &= \mathbb{E}_t[\|\tilde{\boldsymbol{g}}_t - \boldsymbol{g}_t\|^2] + \|\boldsymbol{g}_t\|^2 \\
    &\leq \sigma_{eff}^2 + \|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2.
\end{aligned}
\end{equation}
Plugging these conditional expectations back into the inequality gives
\begin{equation}
\begin{aligned}
    \mathbb{E}_t[\mathcal{L}_\sigma(\boldsymbol{W}_{t+1})]
    &\leq \mathcal{L}_\sigma(\boldsymbol{W}_t) - \eta \|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2 + \frac{L_\sigma\eta^2}{2} \left( \|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2 + \sigma_{eff}^2 \right) \\
    &= \mathcal{L}_\sigma(\boldsymbol{W}_t) - \eta\left(1-\frac{L_\sigma\eta}{2}\right) \|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2 + \frac{L_\sigma\eta^2}{2}\sigma_{eff}^2.
\end{aligned}
\end{equation}
Since $\eta \leq \frac{1}{L_\sigma}$, we have $1-L_\sigma\eta/2 \geq 1/2$. Thus,
\begin{equation}
    \frac{\eta}{2} \|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2 \leq \mathcal{L}_\sigma(\boldsymbol{W}_t) - \mathbb{E}_t[\mathcal{L}_\sigma(\boldsymbol{W}_{t+1})] + \frac{L_\sigma\eta^2}{2}\sigma_{eff}^2.
\end{equation}
Taking the total expectation and summing over $t=1,\ldots,T$, the loss terms telescope:
\begin{equation}
\begin{aligned}
    \frac{\eta}{2}\sum_{t=1}^{T} \mathbb{E}\left[\|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2\right]
    &\leq \sum_{t=1}^{T} \left( \mathbb{E}[\mathcal{L}_\sigma(\boldsymbol{W}_t)] - \mathbb{E}[\mathcal{L}_\sigma(\boldsymbol{W}_{t+1})] \right) + \frac{L_\sigma\eta^2T}{2}\sigma_{eff}^2 \\
    &= \mathcal{L}_\sigma(\boldsymbol{W}_1) - \mathbb{E}[\mathcal{L}_\sigma(\boldsymbol{W}_{T+1})] + \frac{L_\sigma\eta^2T}{2}\sigma_{eff}^2 \\
    &\leq \mathcal{L}_\sigma(\boldsymbol{W}_1) - \mathcal{L}_\sigma^* + \frac{L_\sigma\eta^2T}{2}\sigma_{eff}^2,
\end{aligned}
\end{equation}
where the last inequality applies Assumption 2. Dividing both sides by $\eta T/2$ yields the desired bound:
\begin{equation}
    \frac{1}{T} \sum_{t=1}^{T} \mathbb{E}\left[\|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2\right] \leq \frac{2(\mathcal{L}_\sigma(\boldsymbol{W}_1) - \mathcal{L}_\sigma^*)}{\eta T} + \eta L_\sigma \sigma_{eff}^2.
\end{equation}
This completes the proof. Furthermore, by choosing a horizon-dependent constant stepsize $\eta = \min\{\frac{1}{L_\sigma}, \frac{c}{\sqrt{T}}\}$, the bound becomes $\mathcal{O}(\frac{1}{\sqrt{T}})$, which implies convergence to a first-order stationary point in expectation:
\begin{equation}
    \lim_{T \to \infty} \frac{1}{T} \sum_{t=1}^{T} \mathbb{E}\left[\|\nabla \mathcal{L}_\sigma(\boldsymbol{W}_t)\|^2\right] = 0.
\end{equation}
\end{proof}
